...
fun main(){
    ...
    val side = Sides()
    //소개글 작성
    println("===SpartanSUBWAY lv 3.==="); println("")
    ...
    while(isBuying){
        when (readln()) {
            ...
            "1" -> {
                sandwich.desc()
                val order1 = sandwich.select()
                if (order1.first == "처음으로") {
                    d.intro()
                    continue
                }
                else{
                    println(""); println("${order1.first} 1개를 주문 목록에 추가했습니다.")
                    price += order1.second
                    cart += "${order1.first} 1개 --- ${order1.second}원"
                }

            }
            "2" -> {
                drink.desc();
                val order2 = drink.select()
                if (order2.first == "처음으로") {
                    d.intro()
                    continue
                }
                else{
                    println(""); println("${order2.first} 1개를 주문 목록에 추가했습니다.")
                    price += order2.second
                    cart += "${order2.first} 1개 --- ${order2.second}원"

                }
            }
            "3" -> {side.desc();
                val order3 = side.select()
                if (order3.first == "처음으로") {
                    d.intro()
                    continue
                }
                else{
                    println(""); println("${order3.first} 1개를 주문 목록에 추가했습니다.")
                    price += order3.second
                    cart += "${order3.first} 1개 --- ${order3.second}원"
                }
            }
        }
        d.more()
